...
91 backend tests, plus 3 Playwright end-to-end tests that run against a real browser and real running servers, covering login/role-redirects, draft creation, and the full requester-submits → reviewer-approves flow. `.github/workflows/ci.yml` runs backend lint and tests, frontend lint and build, and the E2E suite on every push. Both linters caught real issues the first time I ran them, including a misconfigured `pyproject.toml` that was silently letting ruff run its full default rule set instead of the one I actually wanted.
