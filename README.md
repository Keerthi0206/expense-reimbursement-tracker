...
- Automated test suite (`backend/tests/`, 91 tests across `test_workflow.py`, `test_admin.py`, `test_error_handling_and_authorization.py`, and `test_email.py`, covering the full reimbursement workflow, validation, RBAC, approval reversal, resubmission after rejection, cancellation, the two-tier approval workflow, duplicate-request detection, budget-limit warnings, receipt OCR extraction and consistency checks, request reminders, notification cleanup, cursor pagination, multi-value status filtering, rate limiting, email notifications, the reviewer-claims-request transition, request-info/resubmission, admin user/role/status management with audit history, sorting and pagination on both list endpoints, the standalone request-history endpoint, a real concurrency race condition found and fixed during development, an N+1 query regression test, a route-ordering regression test, and dedicated coverage of invalid-workflow-action prevention, unauthorized-action handling, and graceful error handling — including SQL-injection-style input)
