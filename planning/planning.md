...
First to drop: the admin UI (API stays, no screen). Next: notifications UI (data still generated
server-side). Last thing to cut: the core Create→Submit→Review→Approve/Reject→Paid workflow,
backend validation, and RBAC — these are the 30%+10%+10% of the rubric that matter most and
won't be sacrificed for polish.
